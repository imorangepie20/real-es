...
- 브랜드 적용(로고·파비콘·빈 상태): 사이드바 헤더를 템플릿 기본("Shadcn UI Kit" + `Command` 아이콘)에서 **real-es 하우스 로고**로 교체, 브라우저 탭 **파비콘**(`src/app/icon.svg`)·메타데이터 title/description 한글화(구 `favicon.ico` 제거). 매물 목록 빈 화면 아이콘을 **하우스 라인 일러스트**로 교체(`EmptyIllustration`, 전체/관심/계약진행/계약완료 공용). 무채색 테마라 전부 모노크롬 SVG(토큰·currentColor)로 라이트/다크 자동 대응. 메뉴 항목 아이콘은 lucide 유지([CLAUDE.md](CLAUDE.md) §6 일관성).
