...
- 매물 색상 태그(관리자 특성): **전체 매물** 메뉴에서 각 행 관심(별) 옆 색 스와치를 클릭해 **4색(보라·파랑·주황·회색)** 지정 → 행 배경이 그 색으로 틴트되고 저장됨(되돌리려면 '없음'). "전체 매물" 제목 아래의 작은 4색 점으로 해당 색만 **필터**. 색상 기능은 전체 매물 메뉴에서만 노출.
