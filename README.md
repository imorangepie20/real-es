...
- 매물 검색 ②(비단지형 클러스터): 비단지형(상가/빌라/토지 등) 지도를 **클러스터(원 안 숫자=묶음 매물 수, `article/map/articleClusters`)** 로 표시, 원 클릭 시 하위 매물(`article/clusteredArticles`) 드릴다운 + "전체 매물 보기". 레이아웃을 **지도(위 박스)+매물(아래 박스)** 로 통일. 동 선택 시 전체 매물도 표에 바로 표시. 신규 스크레이퍼/파서/액션 + 파서 테스트(`articleClusters` 픽스처). 라이브 검증은 prod 재빌드 후 필요.
