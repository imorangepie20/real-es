...
- 대시보드 푸터 추가: `AppFooter`(© 2026 RESM · 부동산 매물 수집·관리 도구)를 `(dashboard)/layout.tsx` 하단에 마운트, 본문과 사이에 여백(mt-4) + 상단 구분선. 헤더와 동일 토큰(border·muted·px-4).
...
- 고객 폼 우편번호 검색: 고객 입력 폼에 **"주소 검색"**(Daum 우편번호 서비스, 무료) 추가 — 검색·선택 시 우편번호+도로명주소 자동 입력, 상세주소는 이어서 직접 입력. 고객에 `zipcode`(우편번호) 필드 추가(비파괴). ⚠️ 외부 CDN이라 prod 라이브에서 동작 확인 필요.
- 일정관리 일정 상세보기: 캘린더에서 **일정을 직접 클릭**하면(월 그리드 칩·날짜 패널 모두) **상세보기**(제목·날짜·시간·유형·연결 매물/고객·메모)가 뜨고, 거기서 **수정·삭제**, 매물 클릭 시 매물로 이동. 매물 파생 일정 클릭 → 해당 매물로 이동.
- 일정관리 상태 유지: 캘린더에서 매물 일정 클릭 → 매물로 이동 후 **뒤로가기 하면 보던 달·선택 날짜가 그대로 복원**됨(이전엔 오늘 달로 리셋). 캘린더 보던 달·선택일을 URL 쿼리에 반영.
- 계약진행 메뉴: 사이드바 `매물 관리`에 **계약진행**(관심매물·계약완료 사이) 추가 — `status="계약진행"` 매물만 보는 전용 화면(`/dashboard/properties/progress`). 엑셀입력·새매물·상태필터 숨김(계약완료 뷰와 동일).
- 엑셀 업로드 시 고객 자동 등록: 매물 엑셀을 올리면 **고객명이 있는 매물은 고객도 자동 생성**되고, 고객유형이 거래유형으로 자동 지정됨 — 매매→매도인, 전세/월세/단기임대→임대인(없으면 단순방문). 이름+전화 같은 고객은 중복 생성 안 함. 업로드 직후 고객관리에 바로 뜨고, 매물 고객명 옆 '미등록' 아이콘도 사라짐.
- 매물↔고객 다대다(역할): **한 매물에 매도인·매수인 등 여러 당사자**, **한 고객이 여러 매물** 연결 가능. 매물 수정 화면 하단 "당사자" 섹션에서 역할(매도인/매수인/임대인/임차인)+이름+전화로 추가·삭제(같은 이름+전화는 고객 1명으로 재사용). 엑셀 업로드 시 소유자측 자동 연결(매매→매도인, 전세/월세/단기임대→임대인). 고객관리·고객 폼에 **연결된 매물 전부+역할** 표시.
- 삭제 확인 다이얼로그 템플릿 통일: 고객 삭제·캘린더 일정 삭제의 브라우저 기본 `confirm` 팝업을 앱 디자인의 스타일 다이얼로그(AlertDialog)로 교체.
- 계약 서류 체크리스트 전체 체크: 계약진행 화면의 "서류 체크리스트" 헤더에 **전체 체크/전체 해제** 버튼 추가(해당 매물 유형의 모든 항목을 한 번에).
- 매물 검색: 매물 목록 상단에 검색창 추가 — **매물명·단지·주소·고객명·전화**로 즉시 필터(기존 유형/거래/상태 필터와 함께 적용). 전체/관심/계약진행/계약완료 모든 뷰 공통.
- 매물 색상 태그(관리자 특성): **전체 매물** 메뉴에서 각 행 관심(별) 옆 색 스와치를 클릭해 **4색(보라·파랑·주황·회색)** 지정 → 행 배경이 그 색으로 틴트되고 저장됨(되돌리려면 '없음'). "전체 매물" 제목 아래의 4색 점으로 해당 색만 **필터**. 색상 기능은 전체 매물 메뉴에서만 노출.
- 반응형 리디자인(매물·고객·실거래·일정): 좁은 화면(< lg)에서 **넓은 표 → 카드 리스트**(매물·고객·실거래 내역), 데스크탑(lg↑)은 기존 표 유지. 헤더 툴바(검색·필터·버튼)는 모바일에서 세로로 정리(검색창 풀폭). 실거래 **지도 높이 반응형**(모바일↓). 일정은 모바일에서 사이드(유형 필터·선택일 패널)가 **달력 아래로** 스택. 기존 템플릿 디자인·색은 유지하고 레이아웃만 반응형화.
- 매물 목록 페이징: 전체/관심/계약진행/계약완료 모든 매물 목록에 **50개/페이지** 페이징(하단 이전/다음·현재 페이지). 검색·필터 변경 시 1페이지로 리셋. 표·모바일 카드 공통.
- 로그인/회원가입 한글화·정비: 로그인·회원가입 페이지 전부 한국어로. **소셜 로그인 버튼 제거**. 회원가입에서 **이름 필수**, 신규 입력 **중개사무소 주소·중개사무소 전화번호·개인 전화번호**(주소·전화는 선택). (비밀번호 찾기 등 나머지 인증 페이지는 영문 유지 — 필요 시 별도 요청.)
- 회원가입 입력 보강: 전화번호 **자동 하이픈** 포맷(중개사무소·개인), 중개사무소 주소 **우편번호 검색기**(Daum), **비밀번호 확인** 일치 검사, **이용약관 동의(필수)** 체크박스(미동의 시 가입 비활성).
- 약관 페이지: **이용약관(/terms)·개인정보처리방침(/privacy)** 페이지 신설(비로그인 접근 가능). 회원가입 동의 문구의 "이용약관"·"개인정보 수집·이용"이 새 탭으로 링크됨. (표준 템플릿 — 실제 운영 전 법무 검토 권장.)
- 사이드바 메뉴 재구성: 업무 중심 6개 그룹으로 정리 — **매물 관리**(전체/관심/계약 진행/계약 완료)·**국토부 실거래가**(조회 및 통계)·**네이버 부동산**(매물 검색/관심 매물)·**고객 관리**(고객 목록)·**일정**(일정 관리)·**설정**(회원 관리·환경 설정·권한 관리). 템플릿 데모 그룹(Dashboards·Apps·AI Apps·Pages·Others)은 사이드바·⌘K에서 제외(페이지 파일은 보존). 미구현 메뉴(회원 관리·권한 관리)는 빈 링크(`#`). 메뉴 단일소스 [src/lib/nav.ts](src/lib/nav.ts).
- 브랜드 적용(로고·파비콘·빈 상태): 사이드바 헤더를 템플릿 기본("Shadcn UI Kit" + `Command` 아이콘)에서 **RESM 하우스 로고**로 교체, 브라우저 탭 **파비콘**(`src/app/icon.svg`)·메타데이터 title/description 한글화(구 `favicon.ico` 제거). 매물 목록 빈 화면 아이콘을 **하우스 라인 일러스트**로 교체(`EmptyIllustration`, 전체/관심/계약진행/계약완료 공용). 무채색 테마라 전부 모노크롬 SVG(토큰·currentColor)로 라이트/다크 자동 대응. 메뉴 항목 아이콘은 lucide 유지([CLAUDE.md](CLAUDE.md) §6 일관성).
...
- 사이드바 로고·RESM 워드마크에 홈(`/real-estate`) 링크 + 호버 피드백.
- 홈 실거래 섹션 강화: 우리 지역 실거래를 작은 카드에서 **큰 섹션**(요약 카드 4·비중 배지·월별 추이 그래프·면적 분포 차트·최근 거래 표)으로 확대. realprice `StatsPanel` 재사용, 본인 주소 법정동의 최근 6개월 매매 기준.
- 매물 색상 태그 행 배경(틴트) 짙게 — 라이트 200·다크 /70. 스와치(dot)는 600 유지.
- 우편번호 검색 팝업 → 다이얼로그 embed: `PostcodeSearch`가 Daum SDK 팝업(`.open()`) 대신 Dialog 안에 검색 UI를 embed — 팝업 차단 환경·데스크탑(Tauri 웹뷰)에서도 동작. 빈 모달 버그(다이얼로그 DOM 늦은 마운트)는 ref 콜백 방식으로 해결, Playwright E2E(검색→선택→폼 자동입력) 검증.
- 매물 일정 그룹을 실거래 흐름(협의→계약→중도금→잔금→입주·기간)으로 재구성 — 대금(계약금·중도금1/2·잔금 금액)과 임대차 만기일 필드 추가, 거래유형별 조건 노출(중도금=매매만·만기일=임대차만), 매매 대금 합계 불일치 안내, 계약서 인쇄에 계약금·중도금·잔금·만기일 자동기입, 캘린더에 만기일 파생 일정. ⚠️ 엑셀 헤더 "중도금1"은 이제 금액(날짜는 "중도금1일").
