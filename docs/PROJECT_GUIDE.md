...
- 일정관리 상태 URL 보존: `calendar/page.tsx`(서버)가 `searchParams`(y·m·d 0-indexed·YYYYMMDD)를 파싱해 `CalendarView`에 `initialView`/`initialDate` prop으로 전달(useSearchParams Suspense 회피). `calendar-view.tsx`는 view·selectedDate 변경 시 **`window.history.replaceState`** 로 URL만 갱신(리렌더·서버 재요청 없음 — router.replace의 RSC 재요청 회피). 매물 일정 클릭 → `router.push(매물)` → 뒤로가기 시 서버 page가 URL 쿼리로 1회 재실행되어 보던 달·선택일 복원. (state는 마운트 후 source of truth, URL은 마운트 시 source of truth.)
