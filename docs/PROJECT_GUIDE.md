...
- 브랜드 자산(모노크롬 SVG): `src/components/logo.tsx`(`Logo` 하우스 심볼, stroke=currentColor라 chip의 text-primary-foreground 상속) + `src/app/icon.svg`(파비콘, 자기완결 색 `#18181b`/`#fafafa` — favicon은 앱 CSS 토큰을 못 받으므로 하드코딩, 구 `favicon.ico`는 `git rm`) + `src/components/empty-illustration.tsx`(`EmptyIllustration`, 템플릿 `<EmptyMedia>` 기본 변형 슬롯용, size-16·muted). app-sidebar 헤더 `Command`→`Logo`·"real-es", layout.tsx 메타데이터 한글화, property-list 빈 상태 미디어 교체(4뷰 공용). **메뉴 픽토그램은 lucide 통일 유지(§6) — 자작 아이콘 안 함**(스트로크 스타일 불일치 방지). naver·realprice 등 다른 빈 화면도 `EmptyIllustration` 재사용 가능.
