...
- 브랜드 자산(모노크롬 SVG): `src/components/logo.tsx`(`Logo` 하우스 심볼, stroke=currentColor라 chip의 text-primary-foreground 상속) + `src/app/icon.svg`(파비콘, 자기완결 색 `#18181b`/`#fafafa` — favicon은 앱 CSS 토큰을 못 받으므로 하드코딩, 구 `favicon.ico`는 `git rm`) + `src/components/empty-illustration.tsx`(`EmptyIllustration`, 템플릿 `<EmptyMedia>` 기본 변형 슬롯용, size-16·muted). app-sidebar 헤더 `Command`→`Logo`·"RESM", layout.tsx 메타데이터 한글화, property-list 빈 상태 미디어 교체(4뷰 공용). **메뉴 픽토그램은 lucide 통일 유지(§6) — 자작 아이콘 안 함**(스트로크 스타일 불일치 방지). naver·realprice 등 다른 빈 화면도 `EmptyIllustration` 재사용 가능.
- 사이트 표시명: **RESM**(사이드바 워드마크·`layout.tsx` title·푸터). 도메인 `resm.approid.team`과 정합. 레포/프로젝트명은 `real-es` 유지 — 표시명과 프로젝트명을 구분(코드/경로/git은 real-es).
- 설정 메뉴 회원 관리 완성(설계 [superpowers/specs/2026-07-01-admin-settings-design.md](superpowers/specs/2026-07-01-admin-settings-design.md)): `getMember` 액션 추가, `members/new`·`[id]` 페이지(`MemberForm` — customer-form 골격만 차용, 필드는 member 전용: 소속·역할 라디오·비번 초기화 다이얼로그). 목록에 **소속(Agency) 필터** 추가, 모듈 스코프 `handleDelete`/`confirm()` → 컴포넌트 `pending` state + 템플릿 `ConfirmDialog`(`members-actions`의 `resetMemberPassword` UI 노출). member는 User 계정 자체(타인 관리·전역 스코프)라 customer(본인 소유 데이터)와 도메인 다름.
- 환경 설정 API 키 DB 연동: `lib/config/keys.ts` `getApiKey(key, envName)` — `getConfig`(SystemConfig) 우선, 빈값 `process.env` 폴백. 카카오 키(naver·realprice 서버 페이지에서 읽어 prop 전달)·공공데이터(`realprice/fetch`)·VWorld(`realprice/geocode`) 4곳 교체. 환경설정 페이지 안내 문구. ⚠️ 라이브 검증은 한국 prod 서버에서(샌드박스 해외IP 403).
