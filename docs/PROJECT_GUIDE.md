...
- 계약 페이지(⑤, Task 5): `src/app/(dashboard)/dashboard/properties/[id]/contract/` — 서버 `page.tsx`가 `getContractData`로 데이터 조회 후 `ContractClient`에 전달. 클라이언트는 진행률을 `checked`+`fieldFilled`에서 매 렌더 파생(state 아님), `toggleChecklistItem`/`updateProperty`를 낙관적으로 직접 호출(debounce 없음). 의존: `contract-actions.ts`(Task 4)·`contract-forms.ts`(Task 3)·`contract-checklist.ts`(Task 2)·`fields.ts`·`format.ts`.
