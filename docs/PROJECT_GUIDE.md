...
- **네이버 수집 단계 2(진행)**: 검색 = **동 드릴다운**(`complex/region` 단지목록 + `article/list` 매물). 단지명 키워드 검색은 폐기. 법정동 코드는 VWorld로 `LegalDivision`에 적재(`naverCode = emd_cd + "00"` = 네이버 `eupLegalDivisionNumber`). 스크래퍼는 `src/lib/naver/`(fetch: 헤드리스 워밍+`context.request`+429 백오프 / parse / cache). VWorld는 `VWORLD_API_KEY` + 서비스 URL `https://resm.approid.team`. **단일 서버 IP 레이트리밋 주의**(보수적 페이싱·캐싱). 확정 설계: [superpowers/specs/2026-06-23-stage-2-collection-design.md](superpowers/specs/2026-06-23-stage-2-collection-design.md).
