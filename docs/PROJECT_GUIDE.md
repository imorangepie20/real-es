...
- 매물 검색 페이지 단계 ①(레이아웃, 설계 [superpowers/specs/2026-06-24-naver-search-page-design.md](superpowers/specs/2026-06-24-naver-search-page-design.md)): 검색 바 Card + 선택 요약 Badge + 지도/결과 좌우 분할 + 로딩 Skeleton/빈 상태 Empty. 데이터 흐름 불변, 시각·구성만 템플릿 기반으로 완성. 다음 ②지도(단지 마커+비단지 클러스터, 신규 스크레이퍼)→③표 필터→④엑셀 필드선택.
